...
v0.14.0 serves text only: no vision, image or prompt flags and no --max-output-tokens operator cap.
"""
import http.client, io, json, socket, subprocess, sys, textwrap, threading
...
class TestPlacementCLI:
  ...
  def test_defaults_and_legacy_options(self):
    ...
    args = cli.parse_args(['--serve','--warmup','--max_context','64'])
    assert args.serve == 8000 and args.warmup and args.max_context == 64
  ...
  def test_text_server_and_legacy_load_signature(self,tmp_path,placed):
    path, _ = save_llama(tmp_path)
    model = Mock()
    argv = placement_args(path) if placed else ['--model','legacy.gguf']
    with patch.object(cli,'LLMServer') as server:
      load = run_cli(argv+['--serve','12345','--benchmark','0'],model)
    model.warmup.assert_called_once()
    server.assert_called_once()
    assert server.call_args.args[0] == ('',12345) and server.call_args.args[1] is model
...
@contextmanager
def running_server(model, *, tok=None):
  server = LLMServer(('127.0.0.1',0),model,'offline',tok or tokenizer(),Mock(render=Mock(return_value='private prompt')))
...
class TestPlacementHTTP:
  def test_dynamic_mocks_are_not_placement(self):
    from tinygrad.llm.serve import is_placed
    assert not is_placed(Mock()) and not is_placed(Mock(max_context=32)) and is_placed(PlacedStub())

  @pytest.mark.parametrize('stream', [False,True])
  def test_legacy_model_keeps_unvalidated_requests(self,stream):
    # The unplaced path answers exactly as v0.14.0 does: no admission check and no option validation.
    model = Mock(max_context=32,get_start_pos=Mock(return_value=0),generate=Mock(side_effect=lambda *a,**kw: iter((2,999))))
    with running_server(model) as server: status, _, body = request(server,stream=stream,temperature=-1,max_tokens=0)
    assert status == 200 and model.generate.call_args.kwargs == {'temperature':-1.0}
    model.check_available.assert_not_called()
    text = ''.join(c['choices'][0]['delta'].get('content','') for c in events(body) if c['choices']) if stream else \
      json.loads(body)['choices'][0]['message']['content']
    assert text == '2 '

  @pytest.mark.parametrize('stream', [False,True])
  @pytest.mark.parametrize('options,param', [({'temperature':-1},'temperature'),({'temperature':float('inf')},'temperature'),
                                             ({'temperature':'hot'},'temperature'),({'max_tokens':0},'max_tokens'),
                                             ({'max_tokens':1.5},'max_tokens'),({'max_tokens':True},'max_tokens')])
  def test_invalid_placed_options_are_rejected_before_admission(self,stream,options,param):
    model = PlacedStub()
    with running_server(model) as server: status, content_type, body = request(server,stream=stream,**options)
    error = json.loads(body)['error']
    assert status == 400 and content_type == 'application/json' and b'[DONE]' not in body
    assert error['type'] == 'invalid_request_error' and error['param'] == param
    assert model.checks == 0 and model.calls == [] and model.state == 'ready'
  ...
  @pytest.mark.parametrize('stream', [False,True])
  def test_iterators_without_close(self,stream):
    model = PlacedStub()
    model.generate = Mock(side_effect=lambda *a,**kw: iter((2,3,999)))
    with running_server(model) as server:
      status, _, body = request(server,stream=stream,max_tokens=1)
    assert status == 200
    if stream:
  ...
  def test_unrelated_errors_are_not_converted_to_placed_execution_errors(self,stream):
    ...
    handler.server = SimpleNamespace(model=model,template=Mock(render=Mock(return_value='hello')),tok=tokenizer())
    raw = json.dumps({'model':'offline','messages':[],'stream':stream}).encode()
    handler.path, handler.headers, handler.rfile = '/v1/chat/completions', {'Content-Length':str(len(raw))}, io.BytesIO(raw)
