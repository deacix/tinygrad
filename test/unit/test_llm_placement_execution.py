...
# v0.14.0 has no Buffer.host; a PYTHON buffer's zero-copy view reads the same bytes without a device sync.
def host_bytes(buf) -> bytes: return bytes(buf.as_memoryview(force_zero_copy=True,no_sync=True))
  ...
  def test_coordinator_uploads_retain_sources_until_sync(self,tmp_path,counts,warmup):
    ...
    def capture(src,device,*args,**kwargs):
      if src.device == 'PYTHON' and device in model.placement.devices:
        ...
        pending.append((device,weakref.ref(buf),host_bytes(buf)))
...
class TestPlacedLifecycle:
  ...
  @pytest.mark.parametrize('kwargs', [{'chunk_size':3},{'temperature':-1},{'temperature':float('inf')}])
  ...
  def test_no_multimodal_seams_and_configuration_frozen(self,tmp_path):
    model, _ = load_placed(tmp_path)
    tokens = Tensor([[3]],dtype=dtypes.int32,device='CPU')
    # v0.14.0 ships no embedding or multimodal entry point that could bypass the placed transaction.
    assert not hasattr(model,'forward_embeddings') and not hasattr(model,'_multimodal_decode')
    for kwarg in ('inputs_embeds','position_ids','rope_delta'):
      with pytest.raises(TypeError): model.generate([2],**{kwarg:1})
