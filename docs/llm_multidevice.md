...
Without placement options, existing text and multimodal loading/serving are unchanged.
...
secured gateway. Existing `--max-output-tokens` limits apply to text serving.
Placement cannot be combined with `--vision-dir`, `--image`, or `--prompt`.
Do not invoke the CLI without an explicit local model when testing offline:
legacy mode still offers downloadable model aliases.
...
not poison model state before mutation. HTTP reports unavailable/busy before headers.
...
finish or `[DONE]`. Context exhaustion reports `length`. Legacy streams remain unchanged. A socket disconnect at a completed
token boundary closes cleanly; it cannot cancel an in-flight synchronous kernel.
...
Tests compare independent teacher-forced logits and every valid KV slice, exercise
changed-input eager/capture/replay, compact storage roots, source lifetime, ties,
no-allocation preflight, ownership and real tiny CPU HTTP execution. Mock tests
separately cover protocol/failure behavior. Skips, NULL and token agreement alone
are not numerical proof.

Physical deployment still needs authorized validation of enumeration, bidirectional
copies, GPU kernels/JIT, peak host/device memory, oversized models, measured speed,
shared-link contention and stability. No driver, firmware, IOMMU/ACS change or GPU
operation is part of this offline gate.
